...
// Force block MinGW GDI/User headers
#define _WINUSER_H
#define _WINGDI_H
#define _WINUSER_
#define _WINGDI_
...
static const char *icon_file_paths[COUNT_UI_ICONS] = {
    ...
    [UI_ICON_LINK]       = "./resources/icons/link.png",
    [UI_ICON_VISUALIZER] = "./resources/icons/visualizer.png",
};
...
typedef struct {
    ...
    Tracks tracks;
    int current_track;
    Visualizer_Mode current_visualizer;
    Font font;
    Shader circle;
...
static void fft_render_bars(Rectangle boundary, size_t m)
{
...
// Visualizer 2: Circle
static void fft_render_circle(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    ...
    float max_radius = fminf(boundary.width, boundary.height)/2 * 0.8f;
    
    for (size_t i = 0; i < m; ++i) {
        float t = p->out_smooth[i];
        float angle = (float)i/m * 2 * PI;
        float radius = max_radius * (0.3f + 0.7f * t);
        float hue = (float)i/m;
        Color color = ColorFromHSV(hue*360, saturation, value);
        
        float x = cx + cosf(angle) * radius;
        float y = cy + sinf(angle) * radius;
        DrawCircle(x, y, 5 + t*10, color);
    }
}

// Visualizer 3: Wave
static void fft_render_wave(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    float cell_width = boundary.width/m;
    ...
    for (size_t i = 0; i < m - 1; ++i) {
        float t1 = p->out_smooth[i];
        float t2 = p->out_smooth[i + 1];
        float hue = (float)i/m;
        Color color = ColorFromHSV(hue*360, saturation, value);
        
        Vector2 start = {
            boundary.x + i*cell_width,
            boundary.y + boundary.height/2 - t1 * boundary.height/3
        };
        Vector2 end = {
            boundary.x + (i+1)*cell_width,
            boundary.y + boundary.height/2 - t2 * boundary.height/3
        };
        DrawLineEx(start, end, 3, color);
    }
}

// Visualizer 4: Spectrum
static void fft_render_spectrum(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    float cell_width = boundary.width/m;
    ...
    for (size_t i = 0; i < m; ++i) {
        ...
        Rectangle bar = {
            boundary.x + i*cell_width,
            boundary.y + boundary.height - t * boundary.height,
            cell_width * 0.8f,
            t * boundary.height
        };
...
// Visualizer 5: Particles
static void fft_render_particles(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    float cell_width = boundary.width/m;
    ...
    for (size_t i = 0; i < m; ++i) {
        ...
        int particle_count = (int)(t * 20);
        for (int j = 0; j < particle_count; ++j) {
            float x = boundary.x + i*cell_width + cell_width/2;
            float y = boundary.y + boundary.height - (float)j/particle_count * t * boundary.height;
            DrawCircle(x, y, 2 + t*3, color);
        }
    }
}

// Visualizer 6: Radial
static void fft_render_radial(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    ...
    float cx = boundary.x + boundary.width/2;
    float cy = boundary.y + boundary.height/2;
    float max_length = fminf(boundary.width, boundary.height)/2 * 0.9f;
    
    for (size_t i = 0; i < m; ++i) {
        float t = p->out_smooth[i];
        float angle = (float)i/m * 2 * PI;
        float length = t * max_length;
        float hue = (float)i/m;
        Color color = ColorFromHSV(hue*360, saturation, value);
        
        Vector2 start = { cx, cy };
        Vector2 end = {
            cx + cosf(angle) * length,
            cy + sinf(angle) * length
        };
...
static void fft_render_mirror(Rectangle boundary, size_t m)
{
    ...
    for (size_t i = 0; i < m; ++i) {
        ...
        float thick = cell_width/3*sqrtf(t);
        DrawLineEx(startTop, endTop, thick, color);
        ...
        DrawLineEx(startBot, endBot, thick, color);
    }
}

// Visualizer 8: Spiral
static void fft_render_spiral(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    ...
    for (size_t i = 0; i < m - 1; ++i) {
        float t1 = p->out_smooth[i];
        float t2 = p->out_smooth[i + 1];
        float angle1 = (float)i/m * 4 * PI;
        float angle2 = (float)(i+1)/m * 4 * PI;
        float radius1 = ((float)i/m) * max_radius * (0.5f + 0.5f * t1);
        float radius2 = ((float)(i+1)/m) * max_radius * (0.5f + 0.5f * t2);
        float hue = (float)i/m;
        Color color = ColorFromHSV(hue*360, saturation, value);
        
        Vector2 start = {
            cx + cosf(angle1) * radius1,
            cy + sinf(angle1) * radius1
        };
        Vector2 end = {
            cx + cosf(angle2) * radius2,
            cy + sinf(angle2) * radius2
        };
        DrawLineEx(start, end, 2 + t1*3, color);
    }
}

// Visualizer 9: Grid
static void fft_render_grid(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    
    int grid_size = (int)sqrtf(m);
    float cell_width = boundary.width / grid_size;
    float cell_height = boundary.height / grid_size;
    
    for (size_t i = 0; i < m && i < grid_size * grid_size; ++i) {
        float t = p->out_smooth[i];
        float hue = (float)i/m;
        Color color = ColorFromHSV(hue*360, saturation, value);
        ...
        Rectangle cell = {
            boundary.x + col * cell_width + cell_width * 0.1f,
            boundary.y + row * cell_height + cell_height * 0.1f,
            cell_width * 0.8f * t,
            cell_height * 0.8f * t
        };
...
// Visualizer 10: Pulse
static void fft_render_pulse(Rectangle boundary, size_t m)
{
    float saturation = 0.75f;
    float value = 1.0f;
    ...
    float cx = boundary.x + boundary.width/2;
    float cy = boundary.y + boundary.height/2;
    float max_radius = fminf(boundary.width, boundary.height)/2 * 0.9f;
    ...
    for (int i = 0; i < bands; ++i) {
        size_t idx = (i * m) / bands;
        if (idx >= m) idx = m - 1;
        float t = p->out_smooth[idx];
        float radius = max_radius * ((float)(i+1)/bands) * (0.5f + 0.5f * t);
        float hue = (float)i/bands;
        Color color = ColorFromHSV(hue*360, saturation, value);
        DrawCircleLines(cx, cy, radius, color);
        DrawCircle(cx, cy, radius * 0.05f, color);
    }
}

// Main render function that dispatches to the current visualizer
static void fft_render(Rectangle boundary, size_t m)
{
    switch (p->current_visualizer) {
        case VIS_BARS:      fft_render_bars(boundary, m); break;
        case VIS_CIRCLE:    fft_render_circle(boundary, m); break;
        case VIS_WAVE:      fft_render_wave(boundary, m); break;
        case VIS_SPECTRUM:  fft_render_spectrum(boundary, m); break;
        case VIS_PARTICLES: fft_render_particles(boundary, m); break;
        case VIS_RADIAL:    fft_render_radial(boundary, m); break;
        case VIS_MIRROR:    fft_render_mirror(boundary, m); break;
        case VIS_SPIRAL:    fft_render_spiral(boundary, m); break;
        case VIS_GRID:      fft_render_grid(boundary, m); break;
        case VIS_PULSE:     fft_render_pulse(boundary, m); break;
        default:            fft_render_bars(boundary, m); break;
    }
}
...
#define visualizer_button(boundary) \
    visualizer_button_with_loc(__FILE__, __LINE__, boundary)
static int visualizer_button_with_loc(const char *file, int line, Rectangle boundary)
{
    uint64_t id = DJB2_INIT;
    id = djb2(id, file, strlen(file));
    id = djb2(id, &line, sizeof(line));
    ...
    DrawTexturePro(p->icon_textures[UI_ICON_VISUALIZER], source, dest, CLITERAL(Vector2){0}, 0, ColorBrightness(WHITE, -0.10));

    // Visualizer names for tooltip
    const char *vis_names[] = {
        "Bars", "Circle", "Wave", "Spectrum", "Particles",
        "Radial", "Mirror", "Spiral", "Grid", "Pulse"
    };
    
    char tooltip_text[64];
    snprintf(tooltip_text, sizeof(tooltip_text), "%s (%d/10)", 
             vis_names[p->current_visualizer], p->current_visualizer + 1);
    tooltip(boundary, tooltip_text, SIDE_TOP, false);
...
static bool toolbar(Track *track, Rectangle boundary)
{
    ...
    // Visualizer Button
    state = visualizer_button((CLITERAL(Rectangle) {
        boundary.x + boundary.width - HUD_BUTTON_SIZE*3,
        boundary.y,
        HUD_BUTTON_SIZE,
        HUD_BUTTON_SIZE,
    }));
    if (state & BS_CLICKED) {
        interacted = true;
        p->current_visualizer = (p->current_visualizer + 1) % COUNT_VISUALIZERS;
    }

    // Link Button
    Rectangle link_boundary = {
        boundary.x + boundary.width - HUD_BUTTON_SIZE*2,
        boundary.y,
        HUD_BUTTON_SIZE,
        HUD_BUTTON_SIZE
    };
    int link_state = button_with_id(UI_ICON_LINK, link_boundary);
    
    Rectangle source = {0, 0, p->icon_textures[UI_ICON_LINK].width, p->icon_textures[UI_ICON_LINK].height};
    Rectangle dest = link_boundary;
    Vector2 origin = {0, 0};

    if (link_state & BS_HOVEROVER) {
        DrawTexturePro(p->icon_textures[UI_ICON_LINK], source, dest, origin, 0, COLOR_HUD_BUTTON_HOVEROVER);
        tooltip(dest, "Load from YouTube", SIDE_TOP, false);
    } else {
        DrawTexturePro(p->icon_textures[UI_ICON_LINK], source, dest, origin, 0, COLOR_HUD_BUTTON_BACKGROUND);
    }
    
    if (link_state & BS_CLICKED) {
        interacted = true;
        const char *url = tinyfd_inputBox("YouTube URL", "Paste YouTube link here:", "");
        if (url) {
            Nob_Cmd cmd = {0};
            // yt-dlp -x --audio-format mp3 -o "./build/%(title)s.%(ext)s" <url>
            // We'll use a fixed name for simplicity or try to capture stdout. 
            // For now, let's just download to a known location.
            nob_cmd_append(&cmd, "yt-dlp", "-x", "--audio-format", "mp3", "-o", "./build/download.%(ext)s", "--force-overwrites", url);
            if (nob_cmd_run_sync(cmd)) {
                 // Check if file exists and load it
                 // We anticipate build/download.mp3
                 const char *dl_path = "./build/download.mp3";
                 Music music = LoadMusicStream(dl_path);
                 if (IsMusicValid(music)) {
                    AttachAudioStreamProcessor(music.stream, callback);
                    char *file_path = strdup(dl_path);
                    nob_da_append(&p->tracks, (CLITERAL(Track) {
                     ...
                     // Maybe it wasn't mp3? fallback or error
                     TraceLog(LOG_ERROR, "Failed to load downloaded audio: %s", dl_path);
                 }
            } else {
                TraceLog(LOG_ERROR, "yt-dlp command failed");
            }
             nob_cmd_free(cmd);
        }
    }
